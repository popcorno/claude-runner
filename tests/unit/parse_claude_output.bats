...
@test "parse_claude_output: extracts result from valid JSON" {
  parse_claude_output '{"result": "hello world", "total_cost_usd": 0.42}'
  [ "$PARSED_RESULT" = "hello world" ]
}

@test "parse_claude_output: extracts cost from valid JSON" {
  parse_claude_output '{"result": "done", "total_cost_usd": 0.42}'
  [ "$PARSED_COST_USD" = "0.42" ]
}

@test "parse_claude_output: handles absent total_cost_usd" {
  parse_claude_output '{"result": "done"}'
  [ "$PARSED_RESULT" = "done" ]
  [ "$PARSED_COST_USD" = "" ]
}

@test "parse_claude_output: handles null total_cost_usd" {
  parse_claude_output '{"result": "done", "total_cost_usd": null}'
  [ "$PARSED_RESULT" = "done" ]
  [ "$PARSED_COST_USD" = "" ]
}

@test "parse_claude_output: handles zero cost" {
  parse_claude_output '{"result": "done", "total_cost_usd": 0}'
  [ "$PARSED_COST_USD" = "0" ]
}
...
@test "parse_claude_output: clears previous values on second call" {
  parse_claude_output '{"result": "first", "total_cost_usd": 1.0}'
...
@test "parse_claude_output: handles empty result field" {
  parse_claude_output '{"result": "", "total_cost_usd": 0.01}'
  [ "$PARSED_RESULT" = "" ]
